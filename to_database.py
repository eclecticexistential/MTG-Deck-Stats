...
def insert_into_who_wins_db():
    for item in who_wins_stats:
        who_wins_db_conn.execute("INSERT INTO Who_wins (Mana, Game, Round, Player, Method) VALUES ('" + str(item[0])
                                 + "', '" + str(item[1]) + "', '" + str(item[2]) + "', '" + str(item[3]) +
                                 "', '" + str(item[4]) + "')")
...
def get_who_wins_stats():
    ...
    try:
        result = cursor.execute("SELECT Mana, Game, Round, Player, Method FROM Who_wins")
        for item in result:
            print(list(item))
...
def initialize_mana_starved():
    ...
    try:
        mana_starved_db_conn.execute(
            ...
            "Game INTEGER NOT NULL, Round INTEGER NOT NULL, Player TEXT NOT NULL);")
        mana_starved_db_conn.commit()
...
def insert_into_mana_starved_db():
    for item in mana_starved:
        mana_starved_db_conn.execute("INSERT INTO Mana_starved (Mana, Game, Round, Player) VALUES ('" + str(item[0])
                                 + "', '" + str(item[1]) + "', '" + str(item[2]) + "', '" + str(item[3]) + "')")
...
def get_mana_starved_stats():
    ...
    try:
        result = cursor.execute("SELECT Mana, Game, Round, Player FROM Mana_starved")
        for item in result:
            print(list(item))
